        ...
        <section className="card">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <MessageCircle className="color-primary" size={20} />
              <h3 style={{ fontSize: '1.25rem' }}>Botão WhatsApp (Para seu Site)</h3>
            </div>
          ...
          <p style={{ fontSize: '0.875rem', opacity: 0.7, marginBottom: '1.5rem' }}>
            Configure um botão flutuante para colocar em seu site principal. Ele inclui a lista de atendentes e captura de lead.
          </p>
            ...
            <div style={{ display: 'grid', gap: '1.5rem' }}>
               ...
               <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                     <label style={{ fontSize: '0.875rem', fontWeight: 500 }}>Seus Atendentes</label>
                     <button className="btn btn-outline" style={{ height: '30px', fontSize: '0.75rem' }} onClick={() => {
                         const wa = settings.whatsappWidget!;
                         const newAt = { id: Math.random().toString(36).substr(2, 9), nome: 'Novo Atendente', cargo: 'Suporte', telefone: '55', disponibilidade: '08:00 às 18:00' };
                         setSettings({...settings, whatsappWidget: {...wa, atendentes: [...wa.atendentes, newAt]}});
                     }}>+ Adicionar Atendente</button>
                  </div>
                  
                  <div style={{ display: 'grid', gap: '1rem' }}>
                     {settings.whatsappWidget.atendentes.map((at, idx) => (
                       <div key={at.id} style={{ padding: '1rem', border: '1px solid #e2e8f0', borderRadius: '8px', background: 'var(--background)', position: 'relative' }}>
                          <button 
                            onClick={() => {
                              const wa = settings.whatsappWidget!;
                          ...
                          <div style={{ display: 'grid', gap: '1rem' }}>
                             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                <div>
                                   <label style={{ fontSize: '0.7rem', opacity: 0.6 }}>Nome</label>
                                   <input 
                                     style={{ width: '100%', height: '32px', borderRadius: '4px', border: '1px solid #e2e8f0', padding: '0 0.5rem', fontSize: '0.85rem' }}
                                     value={at.nome}
                                     onChange={e => {
                                        const wa = settings.whatsappWidget!;
                                ...
                                <div>
                                   <label style={{ fontSize: '0.7rem', opacity: 0.6 }}>WhatsApp (ex: 5548999999999)</label>
                                   <input 
                                     style={{ width: '100%', height: '32px', borderRadius: '4px', border: '1px solid #e2e8f0', padding: '0 0.5rem', fontSize: '0.85rem' }}
                                     value={at.telefone}
                                     onChange={e => {
                             ...
                             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                <div>
                                   <label style={{ fontSize: '0.7rem', opacity: 0.6 }}>Cargo/Setor</label>
                                   <input 
                                     style={{ width: '100%', height: '32px', borderRadius: '4px', border: '1px solid #e2e8f0', padding: '0 0.5rem', fontSize: '0.85rem' }}
                                     value={at.cargo}
                                     onChange={e => {
                                ...
                                <div>
                                   <label style={{ fontSize: '0.7rem', opacity: 0.6 }}>Disponibilidade</label>
                                   <input 
                                     style={{ width: '100%', height: '32px', borderRadius: '4px', border: '1px solid #e2e8f0', padding: '0 0.5rem', fontSize: '0.85rem' }}
